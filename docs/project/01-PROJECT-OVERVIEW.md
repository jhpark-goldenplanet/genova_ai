...
Genova AI는 AI 기반 영상 분석 플랫폼이다. 사용자가 영상을 업로드하면 Google Gemini를 활용하여 자동으로 요약, 자막(스크립트), 구간 분할, 키워드 추출을 수행하고, 다국어 번역까지 제공한다. 교육 기관(AX AgriEdu)의 농업 교육 영상 관리를 1차 타겟으로 설계되었다.
...
2. **분석 설정**: AI 자동 분석(기본) 또는 커스텀 분석(사용자 프롬프트)
