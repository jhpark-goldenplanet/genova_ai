...
- **기능**: 영상 전체 분석(프리미엄 모드), 요약 생성, 구간 분할, 키워드 추출
- **설정**: temperature=0.1, JSON 응답 강제

### STT (Speech-to-Text)
- **역할**: 음성 기반 스크립트 추출 (스탠다드 모드)
- **기능**: 영상 음성 → 텍스트 변환 → Gemini로 요약/구간 분할
- **목적**: 토큰 비용 최적화 (영상 전체를 Gemini에 보내지 않음)
...
사용자가 분석 모드를 선택하면, 모드에 따라 다른 파이프라인을 탄다.
...
### Standard 모드 (STT 기반, 토큰 절약)

```
┌──────────────────────┐
│ 2. STT 스크립트 추출 │  음성 → 텍스트 변환
│    (TRANSCRIBE)      │  전체 스크립트 확보
└──────────┬───────────┘
           ▼
┌──────────────────────┐
│ 3. 텍스트 기반 분석  │  스크립트 텍스트를 Gemini에 전달
│    (SUMMARIZATION)   │  → 요약, 키워드, 구간 분할 생성
└──────────┬───────────┘
           ▼
       마무리 → 완료
```

- **장점**: Gemini에 텍스트만 전달하므로 토큰 비용 대폭 절감
- **적합**: 음성 중심 콘텐츠 (강의, 발표, 인터뷰)

### Premium 모드 (영상 전체 분석)

```
┌──────────────────────┐
│ 2. 영상 전체 분석    │  Gemini가 영상을 직접 시청·분석
│    (SUMMARIZATION)   │  → 요약, 키워드, 구간 분할, 스크립트 생성
└──────────┬───────────┘
           ▼
       마무리 → 완료
```

- **장점**: 화면 내 텍스트(OCR), 시각 정보, 장면 전환까지 분석
- **적합**: 시각 정보가 중요한 콘텐츠 (실습 영상, 자료 화면 포함 강의)

### 공통 후처리

```
┌─────────────────┐
│ 마무리          │  결과 DB 저장, 상태 업데이트
